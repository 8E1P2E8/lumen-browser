...
buildNpmPackage rec {
  ...
  installPhase = ''
    ...
    makeWrapper ${nodejs}/bin/npm $out/bin/lumen-browser \
      --run 'RUNTIME_DIR="''${XDG_CACHE_HOME:-$HOME/.cache}/lumen-browser-dev"; mkdir -p "$RUNTIME_DIR"; if [ ! -f "$RUNTIME_DIR/package.json" ]; then cp -r --no-preserve=mode ''$out/share/${pname}/* "$RUNTIME_DIR/"; fi; cd "$RUNTIME_DIR"' \
